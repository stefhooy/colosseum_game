...
- **The cop builds its own platforms too — not just the player.** Player feedback: "the cop must also build platforms to catch you, it can't be just the player... make it more balanced." Real jumps are still tried first, but once stuck without real upward progress for `COP_BUILD_PATIENCE_BY_DIFFICULTY` seconds, the cop places its own stepping-stone platform (`Cop._build_platform_toward_player`) — a real, physics-based `Platform` appended to the same shared list the player builds onto, sized `COP_BUILD_PLAT_W/H` and capped by `COP_BUILD_RISE` so a following normal jump can actually reach it. This is the cop's ROUTINE way of keeping pace now, not a rare fallback — tagged `built_by="cop"` and tinted red-brick (`COP_PLATFORM_FILL/OUTLINE` in `platform.py`) so it's visually obvious whose platform is whose. Capped defensively at `MAX_COP_BUILDS` per run.
...
- Difficulty tuning (relative to player's `speed=260`, `jump_strength=650`, `gravity=1400`):
  - **Easy**: 180 px/s, 0.6s reaction delay, 220px starting gap, 2.0s build patience, 6.0s break patience.
  - **Medium**: 260 px/s, 0.3s reaction delay, 140px starting gap, 1.2s build patience, 4.0s break patience.
  - **Hard**: 310 px/s, 0.1s reaction delay, 80px starting gap, 0.5s build patience, 2.5s break patience.
