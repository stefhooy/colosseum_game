...
### Music timing (post-launch player feedback)
Music used to start the instant the player left the splash screen — meaning it was already playing under the menu, name entry, difficulty select, and map preview, before any real gameplay began. Moved the `play_music()` call from the splash-exit point to the map-preview → `STATE_GAME` transition (right alongside `reset_run`, so it starts exactly when the run's timer does). Still satisfies the browser autoplay policy (needs a user gesture) since that transition is itself triggered by a keypress/click inside `run_map_preview` — just a later one than the very first splash interaction.
...
- **Mouse lookahead** (player feedback: once the camera stopped showing the whole field, standing still to carefully aim a platform meant you couldn't see or reach anything beyond whatever was already in view — the camera only followed player position, and a stationary player meant a stationary camera). Fixed by nudging the camera's follow target toward wherever the mouse is pointing, scaled by `CAMERA_MOUSE_LOOKAHEAD = 0.5` (settings.py) — computed as the mouse's offset from screen-center (in virtual-surface space) times that factor, added to the player's position before calling `camera.follow`. Lets you scout/aim ahead with the mouse alone, no need to physically walk the character there. `0` = old behavior (camera only ever follows the player); `1.0` = camera moves as far as the raw mouse offset allows.
